...
import UIKit
protocol FilterTaskControllerDelegate: class {
    func senDataFilter(tungay: String, denngay: String, matt: String, mact: String)
}

class FilterTaskController: BaseViewController {
    ...
    weak var delegate: FilterTaskControllerDelegate!
    ...
    @IBAction func mattAlertButton(_ sender: UIButton) {
        ...
        alert.addAlertAZSoft(Api: .taskstatus) { (info) in
            self.mattLabel.text = info?.taskstatus[1]
            self.matt = info?.taskstatus[0]
        }
    ...
    @IBAction func denngayAlertButton(_ sender: UIButton) {
    ...
    @IBAction func successButton(_ sender: UIButton) {
        delegate.senDataFilter(tungay: tungayLabel.text ?? "", denngay: denngayLabel.text ??
            "", matt: matt ?? "", mact: mact ?? "")
        self.navigationController?.popViewController(animated: true)
    }
